...
class household():
    def save_data():
        try:
            fileD=open("프로그래밍1\\자료\\household.txt",'a')
            fileD.write("지출\n")
            fileD.write("%s\n" %contend.get('1.0',END))
...
options=household.read_classification("C:\\Users\\박소호\\Documents\\프로그래밍1\\자료\\가계부\\분류_지출.txt")
OptionMenu(app,classification,*options).pack() #classification 변수를 넣은 옵션메뉴를 만든다
Label(app,text='지출 금액:(원)').pack()
money=Entry(app)
money.pack()
Button(app,text='출력',command=household.print).pack()
Label(app,text='내용:').pack()
contend=Text(app)
contend.pack()
file_open=Button(app,text='파일오픈',command=household.open_file).place(x=50,y=50) #파일 오픈 버튼을 지정한 장소에 만든다
contend_delete=Button(app,text="내용삭제",command=household.delete_contend).place(x=130,y=50) #내용삭제 버튼을 지정한 장소에 만든다
Button(app,text="Save",command=household.save_data).pack()
app.mainloop()
