...
def save_input_data(): 
    try:
        input_fileD=open("프로그래밍1\\자료\\가계부\\가계부 수입.txt",'a')
        input_fileD.write("수입\n")
...
def save_output_data(): 
    try:
        output_fileD=open("프로그래밍1\\자료\\가계부\\가계부 지출.txt",'a')
        output_fileD.write("지출\n")
...
def save_money(): 
    global total_price
    money_fileD=open("프로그래밍1\\자료\\가계부\\자산.txt","a")
...
def show_input(): 
    ...
    """fill로 frame2에 사용되는공간을 사용되지 않는공간을 늘린다.
...
def show_cal(m): 
    ...
    today_date=datetime.now() #현재 시간을 변수에 설저장
...
def money():
    ...
    money_fileD=open("프로그래밍1\\자료\\가계부\\자산.txt")
...
def open_file():
    ...
    text_3.insert(INSERT,household_contend)
    household_f.close()
...
#기본 버튼
window = Tk()
window.title("가계부 프로그램")
window.geometry("650x350+500+300")
